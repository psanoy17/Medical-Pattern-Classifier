"""
K-Fold Cross-Validation Comparison for Diabetes Classification
...
from acor_diabetes import SOCHA_ACOR, FNN, objective_function
...
def save_results(cv_results, output_dir):
    ...
    txt_path = os.path.join(output_dir, 'diabetes_kfold_comparison_results.txt')
...
def save_results_to_csv(cv_results, output_dir):
    ...
    # Save to CSV
    csv_path = os.path.join(output_dir, 'diabetes_kfold_all_runs.csv')
...
def create_comparison_plot(cv_results, output_dir):
    ...
    plt.suptitle(f'Diabetes Classification - Baseline ACOR vs Hybrid ACOR-LM\n'
                 f'({cv_results["n_folds"]}-fold CV, {cv_results["n_runs_per_fold"]} runs/fold, Post-hoc Threshold per Fold)',
                 fontsize=12, fontweight='bold')
    
    plt.tight_layout()
    plot_path = os.path.join(output_dir, 'diabetes_kfold_comparison.png')
...
if __name__ == "__main__":
    print("Diabetes Classification - K-Fold Cross-Validation Comparison")
