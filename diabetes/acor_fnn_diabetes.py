...
# No 'id' column in diabetes.csv, so skip dropping
...
print(f"  True Negatives (no diabetes predicted correctly): {cm[0,0]}")
...
print(f"  True Positives (diabetes predicted correctly): {cm[1,1]}")
print(f"Predicted class distribution: {y_pred.tolist().count(0)} predicted no diabetes, {y_pred.tolist().count(1)} predicted diabetes")
print(f"True class distribution: {y_test.tolist().count(0)} actually no diabetes, {y_test.tolist().count(1)} actually diabetes")
...
plot_confusion_matrix(cm, class_names=["No Diabetes", "Diabetes"], save_path=os.path.join(output_dir, "diabetes_confusion_matrix.png"))
...
plt.ylim(0, 1)
plt.title('Diabetes Model Performance Metrics')
plt.ylabel('Score')
for bar, value in zip(bars, metrics):
    plt.text(bar.get_x() + bar.get_width() / 2, value + 0.02, f'{value:.2f}', ha='center', va='bottom')
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'diabetes_metrics_bar_chart.png'))
plt.show()
...
with open(os.path.join(output_dir, 'diabetes_result.txt'), 'w') as f:
    ...
    f.write(f"  False Negatives (diabetes predicted as no diabetes): {cm[1,0]}\n")
    f.write(f"  True Positives (diabetes predicted correctly): {cm[1,1]}\n")
    f.write(f"Predicted class distribution: {y_pred.tolist().count(0)} predicted no diabetes, {y_pred.tolist().count(1)} predicted diabetes\n")
    f.write(f"True class distribution: {y_test.tolist().count(0)} actually no diabetes, {y_test.tolist().count(1)} actually diabetes\n")
    if len(unique_pred) == 1:
