"""
Phase 2C: Validation of Top 5 Configs
Runs k-fold CV with multiple runs per fold on the top configs from the 500-config sweep.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Validate top configs with CV and multiple runs')
    parser.add_argument('--n-splits', type=int, default=2, help='CV folds')
    parser.add_argument('--runs-per-fold', type=int, default=2, help='Runs per fold')
    parser.add_argument('--seed', type=int, default=123, help='Random seed')
    parser.add_argument('--lm-max-cap', type=int, default=5, help='Cap on LM iterations per local search')
    parser.add_argument('--output', type=str, default='scripts/phase2c_top5_validate.csv', help='Output CSV')
    args = parser.parse_args()

    # Load heart disease data
    heart_path = Path(__file__).resolve().parents[1] / 'heart' / 'heart1.dat'
    ...
    # Top 5 configs from 500-config sweep + best n_colonies=5
    configs = [
    ...
    results = []
    start = time.time()
    print(f"Validating {len(configs)} configs with {args.n_splits}-fold CV and {args.runs_per_fold} runs/fold")
    ...
    for idx, cfg in enumerate(configs, 1):
        ...
        cfg_out = cfg.copy()
        cfg_out.update({'mean_accuracy': mean_acc, 'std_accuracy': std_acc, 'n_evals': n_evals})
