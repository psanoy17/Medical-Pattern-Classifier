"""
Phase 2C: Validate optimal config (from heart tuning) on diabetes dataset
Runs k-fold CV with multiple runs per fold using the best config found on heart data.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Validate optimal config on diabetes dataset')
    ...
    parser.add_argument('--output', type=str, default='scripts/phase2c_diabetes_validate.csv', help='Output CSV')
    ...
    results = []
    start = time.time()
    print(f"Validating {len(configs)} configs on diabetes with {args.n_splits}-fold CV and {args.runs_per_fold} runs/fold")
    ...
    for idx, cfg in enumerate(configs, 1):
        ...
        cfg_out = cfg.copy()
        cfg_out.update({'mean_accuracy': mean_acc, 'std_accuracy': std_acc, 'n_evals': n_evals, 'dataset': 'diabetes'})
